...
async function respond(request: Request, context: Context, includeBody: boolean) {
  try {
    const assemblyId = (await context.params).assemblyId;
    const assembly = await service.getAssembly(assemblyId);
    const absolutePath = await service.resolveAssemblyPath(assemblyId);
    const size = Number(assembly.byteSize);
    const requestedRange = request.headers.get("range");
    const range = requestedRange ? parseSingleByteRange(requestedRange, size) : null;
    const download = new URL(request.url).searchParams.get("download") === "1";
    const commonHeaders = {
      "Accept-Ranges": "bytes",
      "Content-Type": assembly.detectedMimeType,
      "X-Content-Type-Options": "nosniff",
      ETag: `"sha256-${assembly.sha256}"`,
      "Cache-Control": "private, max-age=31536000, immutable",
      ...(download
        ? {
            "Content-Disposition": `attachment; filename="shot-plan-${assembly.generationPlanId}-${assembly.id}.mp4"`,
          }
        : {}),
    };
