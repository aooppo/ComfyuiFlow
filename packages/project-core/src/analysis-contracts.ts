...
export const analysisConfirmSchema = z.object({
  manifestHash: z.string().regex(/^[a-f0-9]{64}$/),
  acknowledgeExternalImageUpload: z.literal(true),
  idempotencyKey: z.string().trim().min(16).max(120),
});
export const understandingReviewSchema = z.object({
  decision: z.enum(["ACCEPTED", "REJECTED"]),
  notes: z.string().trim().max(2_000).optional(),
  idempotencyKey: z.string().trim().min(16).max(120),
});
export const understandingCorrectionSchema = z.object({
  facts: AssetUnderstandingFactsSchema,
  acceptCorrection: z.literal(true),
  idempotencyKey: z.string().trim().min(16).max(120),
});
export const understandingApplicationSchema = z.object({
  targetType: z.enum(["PRODUCTION_ASSET_DRAFT", "ASSET_VERSION_FILE_DRAFT"]),
  targetId: z.string().uuid(),
  fieldMappings: z
    .array(z.object({ sourceField: z.string().max(120), targetField: z.string().max(120) }))
    .min(1)
    .max(20),
  idempotencyKey: z.string().trim().min(16).max(120),
});
