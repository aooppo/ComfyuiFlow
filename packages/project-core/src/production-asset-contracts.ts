...
export const createProductionAssetSchema = z.object({
  type: productionAssetTypeSchema,
  name: z.string().trim().min(1).max(120),
  description: z.string().trim().max(4_000).optional(),
  idempotencyKey: z.string().trim().min(16).max(120).optional(),
});
export const createProductionAssetVersionSchema = z.object({
  basedOnVersionId: z.string().uuid().optional(),
  idempotencyKey: z.string().trim().min(16).max(120).optional(),
});
export const assetVersionFileInputSchema = z.object({
...
export const productionAssetRelationInputSchema = z.object({
...
export const characterStateComponentSchema = z.object({
