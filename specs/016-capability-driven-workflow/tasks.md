...
## Phase 18: Dynamic V3 LIVE Closure (zero-call implementation)

- [x] T128 Amend Constitution 2.0.0, FR-046, V3 contract, plan, data model, and quickstart for evidence-scoped READY and separate QA authority in `.specify/memory/constitution.md` and `specs/016-capability-driven-workflow/`
- [x] T129 Add additive V3 QA/authorization/retry schema and migration in `packages/project-core/prisma/schema.prisma` and `packages/project-core/prisma/migrations/202608270046_dynamic_hailuo_v3_qa_closure/migration.sql`
- [x] T130 [US7] Implement server-owned V3 AI-QA consumption, immutable run/result projection, and Worker integration in `packages/project-core/src/capability-v3-qa-service.ts` and `packages/project-core/src/capability-generation-worker-v3.ts`
- [x] T131 [US7] Block V3 Batch confirmation without current QA LIVE/profile/price facts and disclose independent QA/total ceilings in `packages/project-core/src/generation-execution-service.ts` and `apps/project-web/components/storyboards/workflow-planning-panel.tsx`
- [ ] T132 [US7] Enforce one effective Owner decision, retry lineage/monotonic attempt number, normalized Assembly, and persisted V3 QA read projections in `packages/project-core/src/capability-review-service-v3.ts` and `packages/project-core/src/generation-execution-service.ts`
- [x] T133 [US7] Restore the latest persisted V3 Batch and stop terminal polling while preserving retry switch-over in `apps/project-web/app/api/storyboard-versions/[versionId]/workflow-plans/route.ts` and `apps/project-web/components/storyboards/capability-v3-batch-review.tsx`
- [ ] T134 [P] Add V3 QA contract/unit coverage for price/LIVE blocking, consume-before-call, ambiguity, immutable result projection, and no automatic continuation in `tests/contract/generation-execution-v3-api.test.ts` and `tests/unit/capability-v3-qa-service.test.ts`
- [ ] T135 [P] Add PostgreSQL coverage for QA consumption pair, retry lineage/numbering, conflicting Owner decision, source-digest concurrency, and migration/historical compatibility in `tests/integration/capability-workflow-postgres.test.ts`
- [ ] T136 [P] Add browser reload/terminal-poll/retry-switch/player-frame-QA-history/download acceptance in `tests/contract/capability-workflow-ui.test.ts` and `specs/016-capability-driven-workflow/verification.md`
- [ ] T137 Run serial PostgreSQL migration rehearsal, complete Feature 016 regression/security/performance checks, and record zero-call evidence in `specs/016-capability-driven-workflow/verification.md`
- [ ] T138 Perform the separately authorized Test A LIVE preflight and stop for fresh action-time approval in `specs/016-capability-driven-workflow/verification.md`
