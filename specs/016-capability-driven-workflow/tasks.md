...
- [ ] T129 Add additive V3 QA/authorization/retry schema and migration in `packages/project-core/prisma/schema.prisma` and `packages/project-core/prisma/migrations/202608270046_dynamic_hailuo_v3_qa_closure/migration.sql`
- [ ] T130 [US7] Implement server-owned V3 AI-QA consumption, immutable run/result projection, and Worker integration in `packages/project-core/src/capability-v3-qa-service.ts` and `packages/project-core/src/capability-generation-worker-v3.ts`
- [ ] T131 [US7] Block V3 Batch confirmation without current QA LIVE/profile/price facts and disclose independent QA/total ceilings in `packages/project-core/src/generation-execution-service.ts` and `apps/project-web/components/storyboards/workflow-planning-panel.tsx`
...
- [ ] T133 [US7] Restore the latest persisted V3 Batch and stop terminal polling while preserving retry switch-over in `apps/project-web/app/api/storyboard-versions/[versionId]/workflow-planning-panel.tsx` and `apps/project-web/components/storyboards/capability-v3-batch-review.tsx`
