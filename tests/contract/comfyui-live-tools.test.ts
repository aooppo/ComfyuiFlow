...
async function fixture(root: string, requiresComfyOrgAuth = false) {
...
describe("ComfyUI live bridge contract", () => {
  ...
  it("injects an environment-only Comfy credential into extra_data", async () => {
    const fake = await createFakeComfyUi();
    openServers.push(fake);
    const client = new ComfyUiClient(fake.baseUrl, { comfyOrgApiKey: "test-comfy-key-value" });
    const promptId = randomUUID();
    await client.submitWorkflow(promptId, { "1": { class_type: "Text", inputs: {} } });
    expect(fake.promptBodies).toHaveLength(1);
    expect(fake.promptBodies[0]).toMatchObject({
      prompt_id: promptId,
      extra_data: {
        api_key_comfy_org: "test-comfy-key-value",
        comfy_usage_source: "comfyuiflow-spike",
      },
    });
  });

  it("rejects a Partner Node submission before grant consumption when credential is absent", async () => {
    const fake = await createFakeComfyUi();
    openServers.push(fake);
    const root = await mkdtemp(join(tmpdir(), "comfyuiflow-partner-auth-"));
    const { manifest, registry } = await fixture(root, true);
    const authorization = new AuthorizationService(root);
    const service = new ComfyUiExecutionService({
      client: new ComfyUiClient(fake.baseUrl),
      registry,
    ...
    const grant = await authorization.createGrant({
      operation: "COMFYUI_SUBMIT",
      scopeHash: generationScopeHash(submission),
      expiresAt: new Date(Date.now() + 60_000).toISOString(),
    });
    await expect(service.submit({ ...submission, grantId: grant.id })).rejects.toThrow(
